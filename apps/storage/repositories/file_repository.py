...
class FileRepository:
    ...
    def get_by_folder(folder):
        return File.objects.filter(folder=folder, status=FileStatus.ACTIVE).order_by(
            "file_name"
        )

    @staticmethod
    def get_by_id(file_id):
        return File.objects.select_related("owner", "folder").filter(id=file_id).first()

    @staticmethod
    def exists(folder, file_name) -> bool:
        return File.objects.filter(folder=folder, file_name=file_name).exists()
    ...
    def get_queryset(*, owner) -> QuerySet[File]:
        return File.objects.select_related("folder").filter(
            owner=owner, status=FileStatus.ACTIVE
        )

    @classmethod
    def get_deleted_by_id(cls, *, owner, file_id):
        return (
            File.objects.select_related("folder")
            .filter(
                id=file_id,
                owner=owner,
