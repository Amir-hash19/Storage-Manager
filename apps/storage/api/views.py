...
class CreateFolderView(APIView):
    ...
    def post(self, request):
        ...
        serializer = CreateFolderSerialzer(data=request.data)
...
class FolderContentsView(APIView):
    ...
    @extend_schema(
        request=FolderContentsSerializer, responses={200: FolderContentsSerializer}
    )
    def get(self, request, folder_id):

        result = FolderContentService.get_contents(
            owner=request.user, folder_id=folder_id
        )
...
class FolderDeleteView(APIView):
    ...
    @extend_schema(summary="delete folder", description="delete folder by users.")
    def delete(self, request, folder_id):
        FolderDeleteService.delete(folder_id=folder_id, user=request.user)
...
class FolderRestoreView(APIView):
    ...
    @extend_schema(summary="restore folder", description="restore folder by users.")
    def post(self, request, folder_id):

        FolderRestoreService.restore(folder_id=folder_id, user=request.user)

        return Response(
            {"detail": "Folder Restored Successfully."}, status=status.HTTP_200_OK
        )
...
class EmptyTrashView(APIView):
    ...
    @extend_schema(summary="empty trash", description="empty trash by users.")
...
class FolderListView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: FolderListSerializer},
    ...
    def get(self, request):
        ...
        deleted = request.query_params.get("deleted", "false").lower() == "true"
...
class DownloadfileView(APIView):
    ...
    @extend_schema(summary="download file", description="download file by users.")
    def get(self, request, file_id):
        return DownloadFileService.execute(file_id=file_id, user=request.user)


class FileViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GenericViewSet):
...
class FileDeleteView(APIView):
    ...
    @extend_schema(summary="delete file", description="delete file by users.")
    def delete(self, request, file_id):
        FileService.soft_delete(owner=request.user, file_id=file_id)
...
class FileRestoreView(APIView):
    ...
    @extend_schema(summary="restore file", description="restore file by users.")
    def post(self, request, file_id):

        FileService.restore(owner=request.user, file_id=file_id)
