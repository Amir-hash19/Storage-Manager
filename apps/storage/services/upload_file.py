...
class UploadFileService:
    ...
    @classmethod
    @transaction.atomic
    def upload(cls, *, owner, folder_id, uploaded_file):
        folder = FolderRepository.get_by_id_owner(folder_id=folder_id, owner=owner)

        if folder is None:
            raise FolderNotFound()

        exists = FileRepository.exists(folder=folder, file_name=uploaded_file.name)
        ...
        EventBus.publish(FileUploadRequestedEvent(file_id=file.id, temp_path=temp_path))
